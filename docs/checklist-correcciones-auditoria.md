...
| 2026-09-18 | Persona 2 | [B1] | El seed dejó de reutilizar el hash del administrador: cada técnico recibe una variable bcrypt propia. El runner enlaza ambas variables y las aserciones verifican que los tres hashes sean distintos; se documentaron `Tech2026#1` y `Tech2026#2`. | `database/seed/0001_seed_bootstrap.sql`, `database/.env.example`, `database/scripts/run_validation.mjs`, `database/tests/schema_assertions.mjs` | Pruebas Docker completadas: hashes distintos en BD y acceso exitoso con `admin/Admin2026*`, `jperez/Tech2026#1` y `lramirez/Tech2026#2`. |
...
| 2026-09-18 | Persona 2 | [B2][parcial] | Se agregó un read model de intervención con la garantía más reciente mediante `LEFT JOIN`, vigencia calculada en el caso de uso y campos HTTP `warrantyId`, `kind` y `valid`. El frontend muestra garantía vigente/vencida, oculta “Emitir garantía” cuando existe y recarga la fila después de emitirla; se preservaron los puertos existentes y se añadió fallback para fakes/repositorios antiguos. | `backend/internal/repository/intervention_repository.go`, `backend/internal/usecase/intervention_usecase.go`, `backend/internal/transport/http/intervention_handler.go`, `backend/internal/transport/http/intervention_response_test.go`, `frontend/src/services/service_order_service.ts`, `frontend/src/features/service-order/InterventionPanel.tsx` | Diagnósticos estáticos sin errores y `git diff --check` OK; falta ejecutar `go test ./...` y `npm test` en un entorno con Go/Node. |
